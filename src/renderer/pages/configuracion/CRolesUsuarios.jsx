...
import { doc, setDoc, getDoc, collection, onSnapshot, deleteDoc, serverTimestamp } from "firebase/firestore";
import { createUserWithEmailAndPassword, getAuth } from "firebase/auth";
import { initializeApp, deleteApp } from "firebase/app";
import { db, auth } from "../../../firebaseConfig";

import { useAuth, ROLES, ROLES_LABELS, ROLES_DESCRIPCION } from "../../context/AuthContext";

const CRolesUsuarios = () => {
    const navigate = useNavigate();
    const { userData, esSuperAdmin, esAdmin, getRolesAsignables, tienePermiso } = useAuth();
    ...
    const handleNuevoUsuario = async () => {
        ...
        else if (nuevoPassword.length < 6) newErrors.password = "Mínimo 6 caracteres";
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            try {
                ...
                await createUserWithEmailAndPassword(secondaryAuth, nuevoEmail.toLowerCase(), nuevoPassword);
    ...
    const puedeEditarUsuario = (usuario) => {
        ...
        if (esAdmin() && (usuario.rol === ROLES.SUPER_ADMIN || usuario.rol === ROLES.ADMIN)) return false;
    ...
    const getRolBadgeColor = (rol) => {
        switch (rol) {
            case ROLES.SUPER_ADMIN: return "bg-purple-500/20 text-purple-300 border-purple-500/30";
            case ROLES.ADMIN: return "bg-blue-500/20 text-blue-300 border-blue-500/30";
            case ROLES.GESTOR: return "bg-green-500/20 text-green-300 border-green-500/30";
            case ROLES.DIGITADOR: return "bg-yellow-500/20 text-yellow-300 border-yellow-500/30";
            case ROLES.VISOR: return "bg-slate-500/20 text-slate-300 border-slate-500/30";
                        ...
                        <div>
                            ...
                            {usuariosFiltrados.map((usuario) => (
                                ...
                                >
                                    ...
                                    <div className="w-[20%] flex justify-center">
                                        <span className={`px-3 py-1 text-xs font-medium rounded-full border ${getRolBadgeColor(usuario.rol)}`}>
                            ...
                            {usuariosFiltrados.length === 0 && (
                                <div className="text-center text-gray-400 mt-8 py-8">
                                    ...
                                    {filtroEmail ? "No se encontraron usuarios con ese criterio" : "No hay usuarios registrados"}
                                </div>
                            )}
                        </div>
                    }
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            <Textfield
                                label={<>
                                    Email:
                                    {errors.email && <span className="text-red-300 font-black"> - {errors.email}</span>}
                                </>}
                            ...
                            <Textfield
                                label={<>
                                    Nombre:
                                    {errors.nombre && <span className="text-red-300 font-black"> - {errors.nombre}</span>}
                                </>}
                            ...
                            <Textfield
                                label={<>
                                    Contraseña temporal:
                                    {errors.password && <span className="text-red-300 font-black"> - {errors.password}</span>}
                                </>}
                                    ...
                                    {Object.entries(ROLES_LABELS).map(([rol, label]) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                />
                                                <div>
                                                    <span className={`font-medium ${getRolBadgeColor(rol)} px-2 py-0.5 rounded text-sm`}>
                                                        {label}
                                                    </span>
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {Object.entries(ROLES_LABELS).map(([rol, label]) => {
                                        ...
                                        const esAdminLimitado = rol === ROLES.ADMIN && limiteAdminsAlcanzado && editRolOriginal !== ROLES.ADMIN;
                                        return (
                                            <label
                                                key={rol}
                                                className={`flex items-start gap-3 p-3 rounded-lg border transition-colors cursor-pointer ${
                            ...
                            <div className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/10">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-400">
                                        {editActivo ? "El usuario puede acceder al sistema" : "El acceso está bloqueado"}
                                    </p>
                                </div>
                                <button
                    ...
                    <Modal onClickOutside={() => setEliminarModal(false)} className="!max-w-md">
                        <div className="text-center">
                            ...
                            <p className="text-slate-300 text-sm mb-2">
                                ¿Está seguro que desea eliminar a <span className="font-semibold text-white">{editEmail}</span>?
                            </p>
