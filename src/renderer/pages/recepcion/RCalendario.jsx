  ...
  return (
    <div className="h-screen grid grid-cols-[auto_1fr] grid-rows-[auto_1fr] relative">
      ...
      <div className="flex flex-col gap-4 p-4 pb-20 overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="bg-surface-light border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue"
              >
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {monthlyStats.pendingCount > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-yellow-500/20 rounded-lg">
                ...
                <span className="text-sm text-yellow-400">
                  ...
                  <span className="text-xs ml-1 opacity-75">({formatCLP(monthlyStats.pendingTotal)})</span>
                </span>
              </div>
            )}
            ...
            {monthlyStats.overdueCount > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-danger/20 rounded-lg">
                ...
                <span className="text-sm text-danger">
                  ...
                  <span className="text-xs ml-1 opacity-75">({formatCLP(monthlyStats.overdueTotal)})</span>
                </span>
              </div>
            )}
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-success/20 rounded-lg">
              ...
              <span className="text-sm text-success">
                ...
                <span className="text-xs ml-1 opacity-75">({formatCLP(monthlyStats.paidTotal)})</span>
              </span>
            </div>
          </div>
        </div>
        ...
        {/* Calendar Grid */}
        <div className="bg-gradient-card border border-white/10 rounded-xl overflow-hidden shadow-card">
          {/* Days of Week Header */}
            ...
            {DAYS_OF_WEEK.map((day, index) => (
              <div
                ...
                className={`py-3 text-center text-sm font-semibold ${
                  index >= 5 ? "text-slate-400" : "text-white"
                }`}
              >
                {day}
              </div>
                  ...
                  {day && (
                    <>
                      ...
                      <div className={`
                        ...
                        ${isWeekendDate && !isTodayDate ? "text-slate-400" : "text-white"}
                      `}>
                        {day}
                      </div>
      ...
      {dayModalOpen && selectedDay && (
        ...
        >
                  ...
                  return (
                    <>
                      <div className="flex items-center gap-2 mb-3">
                        <div className={`w-3 h-3 rounded-full ${isOverdueDate ? "bg-danger" : "bg-yellow-400"}`} />
                        <h3 className={`text-lg font-semibold ${isOverdueDate ? "text-danger" : "text-yellow-400"}`}>
                          {isOverdueDate ? "Documentos vencidos sin pagar" : "Documentos por vencer"} ({selectedDayDocuments.expiring.length})
                        </h3>
                      </div>
                        ...
                        {selectedDayDocuments.expiring.map((doc, idx) => (
                          ...
                          >
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                  isOverdueDate ? "bg-danger/20 text-danger" : "bg-yellow-500/20 text-yellow-400"
                                }`}>
                                  {getDocTypeShort(doc.tipo)}
                                </span>
                                <span className="text-white font-medium">N° {doc.numeroDoc}</span>
                                {isOverdueDate && (
                            ...
                            <div className="text-right">
                              <div className="text-white font-semibold">{formatCLP(doc.total)}</div>
          ...
          <div className="mt-4 pt-4 border-t border-white/10 flex justify-between items-center">
            <div className="text-sm text-slate-400">
              {selectedDayDocuments.expiring.length + selectedDayDocuments.paid.length} documento(s) en total
            </div>
            <button
              onClick={() => setDayModalOpen(false)}
